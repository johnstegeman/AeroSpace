...
final class AccordionIndicatorManager {
    ...
    /// Uses the primary screen (origin == .zero) for the Y-flip, not NSScreen.main,
    /// because NSScreen.main gives the focused screen's height which is wrong for
    /// windows on non-primary monitors.
