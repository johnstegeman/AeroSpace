...
final class SubscribeCmdArgsTest: XCTestCase {
    func testParseValidEvents() {
        let result = parseSubscribeCmdArgs(["focus-changed", "monitor-changed", "window-routed", "zone-focused", "zone-preset-changed", "zone-layout-changed", "zone-window-count-changed", "mode-changed"].slice)
        switch result {
            case .cmd(let args):
                assertEquals(args.events, Set([.focusChanged, .monitorChanged, .windowRouted, .zoneFocused, .zonePresetChanged, .zoneLayoutChanged, .zoneWindowCountChanged, .modeChanged]))
            case .help, .failure:
            ...
            case .failure(let err):
                let expectedMsg = """
                    ERROR: Can't parse 'unknown-event'.
                           Possible values: (focus-changed|monitor-changed|focused-monitor-changed|focused-workspace-changed|mode-changed|window-detected|window-routed|binding-triggered|zone-focused|zone-preset-changed|zone-layout-changed|zone-window-count-changed)
                    """
