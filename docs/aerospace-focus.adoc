...
--scope <scope>::
Limit the DFS window list to a specific scope. Only valid with `(dfs-next|dfs-prev)`. +
`<scope>` possible values: `(workspace|current-container)`. +
`workspace` traverses all windows in the workspace (the default). +
`current-container` traverses only windows within the immediate parent container of the focused window — useful for cycling within a single accordion stack or zone.
